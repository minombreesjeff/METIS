...
main(int argc, char *argv[])
{
  ...
  printf("Kmetis: Nvtxs: %6d, Nedges: %8d\n", graph.nvtxs, graph.nedges);

  partition = imalloc(graph.nvtxs, "main: partition");
  options[0] = 0;

  numbering = 0;
  KMETIS(&graph.nvtxs, graph.xadj, graph.adjncy, graph.vwgts, graph.ewgts,
...
ReadGraph(GraphType *graph, char *filename)
{
  ...
  for (i=0; i<graph->nvtxs; i++) {
    ...
    if (readvw) {
      vwgts[i] = (int)strtol(oldstr, &newstr, 10);
      oldstr = newstr;
    }
  ...
  if (k != graph->nedges) {
    printf("readgraph: Something wrong with the edges from input file %d %d\n",graph->nedges, k);
    exit(0);
  }
