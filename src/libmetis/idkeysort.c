...
void idkeysort2(int, DKeyValueType *);
...
void idkeysort(idxtype total_elems, DKeyValueType *pbase)
{
  ...
  if (total_elems > MAX_THRESH) {
    DKeyValueType *lo = pbase;
    DKeyValueType *hi = &lo[total_elems - 1];
    stack_node stack[STACK_SIZE]; 
    stack_node *bottom = stack+1, *top = stack + 2;

    stack[1].lo = stack[1].hi = NULL;
    while (STACK_NOT_EMPTY) {
      DKeyValueType *left_ptr;
      DKeyValueType *right_ptr;
      DKeyValueType *mid = lo + ((hi - lo) >> 1);
...
jump_over:;
  ...
  {
    DKeyValueType *end_ptr = &pbase[total_elems - 1];
    DKeyValueType *tmp_ptr = pbase;
    DKeyValueType *thresh = (end_ptr < pbase + MAX_THRESH ? end_ptr : pbase + MAX_THRESH);
    register DKeyValueType *run_ptr;
...
void idkeysort2(idxtype total_elems, DKeyValueType *pbase)
{
  DKeyValueType stmp;
  DKeyValueType *end_ptr = &pbase[total_elems - 1];
  DKeyValueType *tmp_ptr = pbase;
  DKeyValueType *thresh = end_ptr;
  register DKeyValueType *run_ptr;
