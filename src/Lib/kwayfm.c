 ...
 * George
 *
 * $Id: kwayfm.c,v 1.2 1997/12/22 19:54:47 karypis Exp $
 *
 */
        ...
        *======================================================================*/
        ...
        for (j=xadj[i]; j<xadj[i+1]; j++) {
          ...
          if (me == from) {
            ...
            /* First decrease and the increase */
            for (k=0; k<myrinfo->ndegrees; k++) {
              if (myedegrees[k].pid == from) {
                if (myedegrees[k].ed == adjwgt[j])
                  myedegrees[k] = myedegrees[--myrinfo->ndegrees];
...
/*************************************************************************
* This function performs k-way balancing 
**************************************************************************/
void Random_KWayEdgeBalance(CtrlType *ctrl, GraphType *graph, int nparts, float *tpwgts, float ubfactor, int npasses)
{
  int i, ii, iii, j, jj, k, l, pass, nvtxs, nmoves, nbnd, tvwgt, myndegrees; 
    ...
    for (iii=0;;iii++) {
      ...
      *======================================================================*/
      ...
      for (j=xadj[i]; j<xadj[i+1]; j++) {
        ...
        if (me == from) {
          if (myrinfo->ed >= myrinfo->id && bndptr[ii] == -1)
            BNDInsert(nbnd, bndind, bndptr, ii);
