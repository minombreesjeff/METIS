 ...
 * George
 *
 * $Id: edge_refine.c,v 1.2 1997/07/18 00:32:06 karypis Exp $
 */
...
void KWayRefine(CtrlType *ctrl, GraphType *graph, WorkSpaceType *wspace, int npasses, float ubfraction)
{
  ...
  GKfree(&update, &nupds_pe, &htable, &changed, &pperm, -1);
...
void KWayRefineClean(CtrlType *ctrl, GraphType *graph, WorkSpaceType *wspace, int npasses, float ubfraction)
{
  ...
  GKfree(&update, &nupds_pe, &htable, &changed, &pperm, &perm, -1);
...
void KWayAdaptiveRefineClean(CtrlType *ctrl, GraphType *graph, WorkSpaceType *wspace, int npasses, float ubfraction)
{
  ...
  for (pass=0; pass<npasses; pass++) {
    ...
    for (c=0; c<2; c++) {
      ...
      IFSET(ctrl->dbglvl, DBG_REFINEINFO, rprintf(ctrl, "Actual done: %d %d\n", GlobalSESum(ctrl, ndone), GlobalSEMax(ctrl, ndone)));
      ...
      /* Finally, sum-up the partition weights */
      if (GlobalSESum(ctrl, ndone) > 0.2*badminpwgt)
        MPI_Allreduce((void *)lpwgts, (void *)gpwgts, nparts, IDX_DATATYPE, MPI_SUM, ctrl->comm);
    }
  ...
  MPI_Allreduce((void *)lpwgts, (void *)gpwgts, nparts, IDX_DATATYPE, MPI_SUM, ctrl->comm);
  GKfree(&update, &nupds_pe, &htable, &changed, &perm, &pperm, &tocheck, -1);
