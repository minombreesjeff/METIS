 ...
 * George
 *
 * $Id: order.c,v 1.4 1997/07/18 00:32:11 karypis Exp $
 *
 */
...
void MultilevelOrder(CtrlType *ctrl, GraphType *graph, idxtype *order, idxtype *sizes, WorkSpaceType *wspace)
{
  ...
  LocalOrder(ctrl, mgraph, morder, lastnode[2*(ctrl->npes+ctrl->mype)]-mgraph->nvtxs, wspace);
  ...
  FreeGraph(mgraph);
  GKfree(&perm, &lastnode, &porder, &morder, -1);
...
void CompactGraph(CtrlType *ctrl, GraphType *graph, idxtype *perm, WorkSpaceType *wspace)
{
  ...
  **************************************************************/
  ...
  GKfree(&graph->match, &graph->cmap, &graph->lperm, &graph->where, &graph->label, &graph->rinfo,
         ...
         &graph->peadjncy, &graph->peadjloc, -1);
...
**************************************************************************/
void LocalOrder(CtrlType *ctrl, GraphType *graph, idxtype *order, int firstnode, WorkSpaceType *wspace)
{
  ...
  /* Relabel the vertices so that they are in local index space, and start from 1 */
  for (i=0; i<nvtxs; i++) {
    for (j=xadj[i]; j<xadj[i+1]; j++) {
  ...
  ASSERT(ctrl, 6*(nvtxs+5) < wspace->maxcore);
  ...
  for (i=0; i<nvtxs; i++) {
    ASSERT(ctrl, iperm[i]-1>=0 && iperm[i]-1<nvtxs);
    order[i] = firstnode+iperm[i]-1;
  }
