 ...
 * George
 *
 * $Id: drivers.c,v 1.5 1997/07/18 00:32:05 karypis Exp $
 *
 */
...
void Global_Partition(CtrlType *ctrl, GraphType *graph, WorkSpaceType *wspace)
{
  ...
  if (graph->gnvtxs < ctrl->CoarsenTo || SmallerSubGraph(ctrl, graph) < 10 || (graph->finer != NULL && graph->gnvtxs > graph->finer->gnvtxs*COARSEN_FRACTION)) {
  ...
  if (graph->gnvtxs < ctrl->CoarsenTo || SmallerSubGraph(ctrl, graph) < 10 || (graph->finer != NULL && graph->gnvtxs > graph->finer->gnvtxs*COARSEN_FRACTION)) {
    ...
    if (gnedges-ctrl->foldf < (int)(0.3*ctrl->foldf) && EnoughMemory(ctrl, graph->nedges, wspace->nlarge>>1)) {
      ...
      if (ctrl->mype%2 == 0) { /* Even-numbered processors proceed only */
        ...
        if (ctrl->npes%2 == 0 && ctrl->npes > 2)  /* Even # of processors */
...
void Order_Partition(CtrlType *ctrl, GraphType *graph, WorkSpaceType *wspace)
{
  ...
  if (graph->gnvtxs < ctrl->CoarsenTo || SmallerSubGraph(ctrl, graph) < 5 || (graph->finer != NULL && graph->gnvtxs > graph->finer->gnvtxs*COARSEN_FRACTION)) {
