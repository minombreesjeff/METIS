 ...
 * George
 *
 * $Id: move.c,v 1.2 1997/07/18 00:32:10 karypis Exp $
 *
 */
...
GraphType *MoveGraph(CtrlType *ctrl, GraphType *graph, WorkSpaceType *wspace)
{
  ...
  for (i=0; i<nvtxs; i++) {
    ASSERT(ctrl, where[i] >= 0 && where[i] < ctrl->npes);
    sinfo[where[i]].key++;
...
*/
  ...
  if (lpwgts[nparts]+gpwgts[nparts] > wspace->maxcore)
    myprintf(ctrl, "Too much memory during movement!\n");
...
* The routine assumes that npes == nparts.
**************************************************************************/
