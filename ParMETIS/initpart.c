 ...
 * George
 *
 * $Id: initpart.c,v 1.2 1997/07/18 00:32:07 karypis Exp $
 */
...
void InitPartition(CtrlType *ctrl, GraphType *graph, WorkSpaceType *wspace, int context)
{
  ...
  Ser_KMetis(agraph, ctrl->nparts, UNBALANCE_FRACTION);
  ...
  if (lpecut[1] == gpecut[1] && gpecut[1] != 0) 
    MPI_Send((void *)agraph->where, agraph->nvtxs, IDX_DATATYPE, 0, 1, ctrl->gcomm);
  if (lpecut[1] == 0 && gpecut[1] != 0)
    MPI_Recv((void *)agraph->where, agraph->nvtxs, IDX_DATATYPE, gpecut[1], 1, ctrl->gcomm, &ctrl->status);
...
void InitPartition_RB(CtrlType *ctrl, GraphType *graph, WorkSpaceType *wspace, int context)
{
  ...
  MPI_Comm_rank(ctrl->gcomm, &mypartnum);

  gnvtxs = agraph->nvtxs;

  /* Go into recursive partition */
  ndim = log2(ctrl->nparts);
  for (i=0; i<ndim; i++) { 
    Ser_MlevelBisectGraph(agraph, 1.01, part);
    KeepPart(ctrl, agraph, wspace, part, (mypartnum>>i)%2);
  }

  /* Let every processor know the global partitioning vector */
  gwhere0 = idxsmalloc(gnvtxs, 0, "InitPartition: gwhere0");
  gwhere1 = idxmalloc(gnvtxs, "InitPartition: gwhere1");

  for (i=0; i<agraph->nvtxs; i++) 
    gwhere0[agraph->label[i]] = mypartnum;

/*
  myprintf(ctrl, "Mypart: %d, Gnvtxs: %d, Anvtxs: %d [%d]\n", mypartnum, gnvtxs, agraph->nvtxs, idxsum(agraph->nvtxs, agraph->vwgt));
*/
