 ...
 * George
 *
 * $Id: node_refine.c,v 1.2 1997/07/18 00:32:10 karypis Exp $
 */
...
**************************************************************************/
void ComputeNodePartitionParams0(CtrlType *ctrl, GraphType *graph, WorkSpaceType *wspace)
{
  int i, j, k, l, nparts, nvtxs, firstvtx, lastvtx;
  idxtype *xadj, *ladjncy, *adjwgt, *vtxdist, *vwgt, *lpwgts, *gpwgts;
  ...
  /* myprintf(ctrl, "%d %d %d %d \n", graph->mincut, graph->lmincut, gpwgts[0], gpwgts[1]); */
...
**************************************************************************/
void KWayNodeRefine0(CtrlType *ctrl, GraphType *graph, WorkSpaceType *wspace, int npasses, float ubfraction)
{
  ...
  idxtype *where, *lpwgts, *gpwgts, *perm;
  ...
  for (pass=0; pass<npasses; pass++) {
    ...
    for (c=0; c<2; c++) {
      ...
      for (ii=0; ii<nvtxs; ii++) {
        ...
        /* Go through the loop if a separator vertex and gain is possible */
        if (from >= nparts && rinfo[i].edegrees[(c+1)%2] <= vwgt[i]) {
  ...
  GKfree(&update, &nupds_pe, &htable, &changed, &perm, -1);
...
void KWayNodeRefine(CtrlType *ctrl, GraphType *graph, WorkSpaceType *wspace, int npasses, float ubfraction)
{
  ...
  GKfree(&update, &nupds_pe, &htable, &changed, -1);
...
* This function performs k-way refinement
**************************************************************************/
void KWayNodeRefine2(CtrlType *ctrl, GraphType *graph, WorkSpaceType *wspace, int npasses, float ubfraction)
{
  int i, ii, j, k, l, pass, nvtxs, firstvtx, lastvtx, otherlastvtx, c, nmoves, 
      nlupd, nsupd, nnbrs, nchanged, nunion;
  int npes = ctrl->npes, mype = ctrl->mype, nparts = ctrl->nparts;
  idxtype *xadj, *ladjncy, *adjwgt, *vtxdist, *vwgt;
  ...
  NRInfoType *rinfo, *myrinfo;
  int from, me, other, otherwgt, oldcut;
  idxtype *mark, *scratch, *uset, *perm;
  int i1, j1, k1, pid, uwgt;
  ...
  scratch = idxsmalloc(nvtxs+graph->nrecv, -1, "KWayRefine: scratch");
  mark = idxsmalloc(nvtxs, 0, "KWayRefine: mark");
  uset = idxmalloc(8192, "KWayRefine: uset");
  ...
  for (pass=0; pass<npasses; pass++) {
    ...
    for (c=0; c<2; c++) {
      ...
      for (ii=0; ii<nvtxs; ii++) {
        ...
        /* Check to see if the complicated moves will help */
        if (from >= nparts && rinfo[i].edegrees[(c+1)%2] > vwgt[i]) { 
          pid = from%nparts + ((c+1)%2);  /* The partition id from where you are pulling */

          for (j=xadj[i]; j<xadj[i+1]; j++) {
            k = ladjncy[j];
            /* If it is a local separator vertex that has negative weight see if they can be moved together */
            if (k < nvtxs && k > i && SelectWhere(where[k]) >= nparts && rinfo[k].edegrees[(c+1)%2] > vwgt[k]) { 
              nunion = uwgt = 0;
              for (j1=xadj[i]; j1<xadj[i+1]; j1++) {
                k1 = ladjncy[j1];
                if (SelectWhere(where[k1]) == pid) {
                  if (scratch[k1] != i) {
                    scratch[k1] = i;
                    uwgt += SelectWeight(where[k1]);
                    uset[nunion++] = k1;
                  }
                }
              }
              for (j1=xadj[k]; j1<xadj[k+1]; j1++) {
                k1 = ladjncy[j1];
                if (SelectWhere(where[k1]) == pid) {
                  if (scratch[k1] != i) {
                    scratch[k1] = i;
              ...
              if (uwgt < vwgt[i]+vwgt[k]) {
                mark[i] = mark[k] = 1;
                /* myprintf(ctrl, "Combined move is positive %d %d %d %d %d %d %d %d\n", i, k, vwgt[i], rinfo[i].edegrees[(c+1)%2], vwgt[k], rinfo[k].edegrees[(c+1)%2], uwgt, vwgt[i]+vwgt[k]-uwgt); */
              }
              for (j1=0; j1<nunion; j1++)
                scratch[uset[j1]] = -1;
            }
          }
        }
        ...
        /* Go through the loop if a separator vertex and gain is possible */
        if ((from >= nparts && rinfo[i].edegrees[(c+1)%2] <= vwgt[i]) || mark[i]) { 
          mark[i] = 0;
          other = from%nparts+c;  /* It is one-sided move so we know where it goes */
  ...
  GKfree(&update, &nupds_pe, &htable, &changed, &scratch, &uset, &mark, &perm, -1);
