 ...
 * George
 *
 * $Id: edge_refine.c,v 1.5 1998/09/21 21:16:25 karypis Exp $
 */
...
{
  int i, j, nvtxs, nnbrs, firstvtx, cfirstvtx;
...
{
  int i, j, k, l, nvtxs, firstvtx, lastvtx;
  idxtype *xadj, *ladjncy, *adjwgt, *vtxdist, *lpwgts, *gpwgts;
...
* This function computes the initial id/ed 
**************************************************************************/
void ComputeCut(CtrlType *ctrl, GraphType *graph, WorkSpaceType *wspace)
{
  int i, j, k, l, nvtxs;
...
**************************************************************************/
void KWayRefine(CtrlType *ctrl, GraphType *graph, WorkSpaceType *wspace, int npasses, float ubfraction)
{
  ...
  idxtype *changed, *pperm;
  KeyValueType *swchanges, *rwchanges;
  int *nupds_pe;
  RInfoType *rinfo, *myrinfo;
  EdgeType *degrees;
  int from, me, other, vwgt, badminpwgt, badmaxpwgt, oldcut;
  ...
  IFSET(ctrl->dbglvl, DBG_REFINEINFO, rprintf(ctrl, "K-way refinement %2d [%5d, %5d], [%5d, %5d] Cut: %d, \tBalance: %6.3f\n",
  ...
  for (pass=0; pass<npasses; pass++) {
      ...
      for (i=0; i<nvtxs; i++) {
        if (rinfo[i].ed >= rinfo[i].id) { /* Total ED is too high */
          ...
          if (k < rinfo[i].ndegrees) { /* You actually found one */
            ...
            if (degrees[k].ewgt > rinfo[i].id || (degrees[k].ewgt == rinfo[i].id && (gpwgts[from] - gpwgts[other] >= vwgt))) {
    ...
    IFSET(ctrl->dbglvl, DBG_REFINEINFO, rprintf(ctrl, "\t[%5d, %5d], [%5d, %5d] Cut: %d\n", 
...
void KWayAdaptiveRefineClean(CtrlType *ctrl, GraphType *graph, WorkSpaceType *wspace, int npasses, float ubfraction)
{
  ...
  RInfoType *rinfo, *myrinfo;
  EdgeType *degrees;
  int from, me, other, to, vwgt, vsize, badminpwgt, badmaxpwgt, oldcut, ntodo, ndone;
  ...
  IFSET(ctrl->dbglvl, DBG_REFINEINFO, rprintf(ctrl, "K-way adaptive refinement %2d [%5d, %5d], [%5d, %5d] Cut: %d, \tBalance: %6.3f\n",
        graph->level, gpwgts[idxamin(nparts, gpwgts)], gpwgts[idxamax(nparts, gpwgts)], badminpwgt, badmaxpwgt, graph->mincut,
        1.0*nparts*gpwgts[idxamax(nparts, gpwgts)]/(1.0*idxsum(nparts, gpwgts))));
...
* This function performs k-way refinement
**************************************************************************/
void KWayAdaptiveRefineClean2(CtrlType *ctrl, GraphType *graph, WorkSpaceType *wspace, int npasses, float ubfraction)
{
  int i, ii, j, k, l, pass, nvtxs, firstvtx, lastvtx, otherlastvtx, c, nmoves, 
      nlupd, nsupd, nnbrs, nchanged;
  int npes = ctrl->npes, mype = ctrl->mype, nparts = ctrl->nparts;
  ...
  maxpush = idxmalloc(nparts, "maxpush");
...
*/
      ...
      /* Compute the maximum push that any processors can perform */
      for (i=0; i<nparts; i++) {
        if (gpwgts[i] > badmaxpwgt) {
          maxpush[i] = ((gpwgts[i]-badmaxpwgt)*lpwgts[i])/gpwgts[i];
          if (lpwgts[i]*nparts > gpwgts[i])
            maxpush[i]++;
        }
        else
          maxpush[i] = 0;
...
/*
myprintf(ctrl, "*Node: %3d, Gain: %3d, From: %d, To: %d, Vwgt: %3d, Fwgt: %3d, Twgt: %3d, Max/Min: %d %d [%d]\n", i, degrees[k].ewgt-rinfo[i].id, from, other, vwgt, gpwgts[from], gpwgts[other], badminpwgt, badmaxpwgt, maxpush[from]);
*/
