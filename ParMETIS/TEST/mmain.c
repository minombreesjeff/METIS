 ...
 * George
 *
 * $Id: main.c,v 1.2 1998/09/22 23:17:18 karypis Exp $
 *
 */
...
int main(int argc, char *argv[])
{
  ...
  int mgcnum = -1, mgcnums[5] = {-1, 2, 3, 4, 2};
  ...
  if (argc < 2) {
    if (mype == 0)
      printf("Usage: %s <mesh-file> [MagicNumber]\n", argv[0]);
  ...
  ParMETIS_V3_PartMeshKway(mesh.elmdist, mesh.elements, NULL, &(mesh.etype), &mgcnum,
  &wgtflag, &numflag, &(mesh.ncon), &nparts, tpwgts, ubvec, options, &edgecut, part, &comm);
...
*/ 
  ...
  GKfree((void *)&part, (void *)&tpwgts, LTERM);
  MPI_Comm_free(&comm);
  MPI_Finalize();
  return 0;
}
